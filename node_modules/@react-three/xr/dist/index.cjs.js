...
function useHitTest(hitTestCallback) {
  ...
  var sessionRef = React.useRef(gl.xr.getSession());
  ...
  reactThreeFiber.useFrame(function () {
    ...
    if (hitTestSource.current && gl.xr.isPresenting) {
      var referenceSpace = gl.xr.getReferenceSpace(); // This raf is unnecesary, we should get XRFrame from r3f but it's not implemented yet

      sessionRef.current.requestAnimationFrame(function (_time, xrFrame) {
        var hitTestResults = xrFrame.getHitTestResults(hitTestSource.current);

        if (hitTestResults.length) {
          var _hit = hitTestResults[0];
          hitMatrix.fromArray(_hit.getPose(referenceSpace).transform.matrix);
...
function XR(props) {
  var _useThree3 = reactThreeFiber.useThree(),
      gl = _useThree3.gl,
...
function XRCanvas(_ref) {
  ...
  return /*#__PURE__*/React.createElement(reactThreeFiber.Canvas, _extends({
    vr: true
  }, rest), /*#__PURE__*/React.createElement(XR, null, /*#__PURE__*/React.createElement(InteractionManager, null, children)));
}
...
function ARCanvas(_ref4) {
  var children = _ref4.children,
      ...
      rest = _objectWithoutPropertiesLoose(_ref4, ["children", "sessionInit"]);

  return /*#__PURE__*/React.createElement(XRCanvas, _extends({
    onCreated: function onCreated(_ref5) {
      var gl = _ref5.gl;
      return void document.body.appendChild(ARButton.ARButton.createButton(gl, sessionInit));
    }
  }, rest), children);
}
