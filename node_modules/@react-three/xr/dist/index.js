...
import React__default, { useMemo, useContext, useRef, useEffect, forwardRef, createContext, useState, createElement } from 'react';
...
function useHitTest(hitTestCallback) {
  ...
  const sessionRef = useRef(gl.xr.getSession());
  ...
  useFrame(() => {
    ...
    if (hitTestSource.current && gl.xr.isPresenting) {
      const referenceSpace = gl.xr.getReferenceSpace(); // This raf is unnecesary, we should get XRFrame from r3f but it's not implemented yet
...
export { ARCanvas, DefaultXRControllers, Hands, InteractionManager, InteractionsContext, Interactive, RayGrab, VRCanvas, XR, XRController, useController, useHitTest, useInteraction, useXR, useXREvent };
